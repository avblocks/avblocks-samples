...
void setDefaultOptions(Options& opt)
{
	...
	opt.outputFile = getExeDir() + L"\\equinox-48KHz.au.aac";
}

void help(OptionsConfig<wchar_t>& optcfg)
{
    wcout << L"Using: enc_aac_au_pull --input <wav file> --output <aac file>" << endl;
	doHelp(wcout, optcfg);
}
...
ErrorCodes prepareOptions(Options &opt, int argc, wchar_t* argv[])
{
    ...
    optcfg.addOptions()
        ...
        (L"output,o",   opt.outputFile, wstring(),  L"output raw AAC file (without headers)");
    try
    {
