...
CVideoConverterDlg::CVideoConverterDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CVideoConverterDlg::IDD, pParent)
	, m_strInputFile(_T(""))
	, m_strOutputFile(_T(""))
	, m_strStatus(_T(""))
{
...
void CVideoConverterDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
...
BEGIN_MESSAGE_MAP(CVideoConverterDlg, CDialog)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	//}}AFX_MSG_MAP
	...
	ON_CBN_SELCHANGE(IDC_COMBO_PRESETS, &CVideoConverterDlg::OnCbnSelchangeComboPresets)
END_MESSAGE_MAP()


// CVideoConverterDlg message handlers

BOOL CVideoConverterDlg::OnInitDialog()
{
...
void CVideoConverterDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CVideoConverterDlg::OnPaint() 
{
	if (IsIconic())
	{
...
HCURSOR CVideoConverterDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}

void CVideoConverterDlg::OnBnClickedOk()
{
}

void CVideoConverterDlg::OnBnClickedCancel()
{
	if (m_bWorking)
		return;
...
void CVideoConverterDlg::OnBnClickedButtonChooseInputFile()
{
	UpdateData(TRUE);
	...
	CFileDialog dlg(TRUE, NULL,
		...
		OFN_HIDEREADONLY,
		_TEXT("Video files (*.mp4,*.mpg,*.mpeg,*.mod,*.avi,*.wmv,*.mts,*.m2t,*.ts,*.tod,*.m2v,*.m4v,*.webm,*.dat,*.mpe,*.mpeg4,*.ogm)|*.mp4;*.mpg;*.mpeg;*.mod;*.avi;*.wmv;*.mts;*.m2t;*.ts;*.tod;*.m2v;*.m4v;*.webm;*.dat;*.mpe;*.mpeg4;*.ogm|")
		_TEXT("Image files (*.jpg,*.jpeg,*.png,*.bmp,*.tiff,*.tif,*.gif)|*.jpg;*.jpeg;*.png;*.bmp;*.tiff;*.tif;*.gif|")
		_TEXT("All files (*.*)|*.*||"), 
		NULL);
...
void CVideoConverterDlg::OnBnClickedButtonChooseOutputFile()
{
	UpdateData(TRUE);
...
void CVideoConverterDlg::OnBnClickedButtonStart()
{
	UpdateData(TRUE);
...
void CVideoConverterDlg::OnBnClickedButtonStop()
{
	m_bStop = true;
	UpdateControls();
}

bool CVideoConverterDlg::ValidateInputData()
{
...
void CVideoConverterDlg::UpdateControls()
{
	UpdateData(TRUE);
...
void CVideoConverterDlg::onProgress(double currentTime, double totalTime)
{
	if(totalTime > 0)
	{
...
void CVideoConverterDlg::onStatus(primo::avblocks::TranscoderStatus::Enum status)
{
...
bool_t	CVideoConverterDlg::onContinue(double currentTime)
{
	return !m_bStop;
}

LRESULT CVideoConverterDlg::OnConvertThreadFinished(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CVideoConverterDlg::OnConvertProgress(WPARAM wParam, LPARAM lParam)
{
	UpdateData(TRUE);
...
UINT CVideoConverterDlg::ConvertThread(LPVOID param)
{
	HRESULT hr = CoInitialize(NULL);

	CVideoConverterDlg *dlg = reinterpret_cast<CVideoConverterDlg*>(param);
	dlg->Convert();
...
void CVideoConverterDlg::Convert()
{
	CStringA utf8;
...
void CVideoConverterDlg::OnCbnSelchangeComboPresets()
{
	const PresetDescriptor& preset = GetSelectedPreset();
