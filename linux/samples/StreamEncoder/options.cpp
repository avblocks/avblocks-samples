...
void listPresets()
{
	...
	for (int i=0; i < avb_presets_len; ++i)
	{
		...
		cout << left << setw(30) <<  preset.name << " ." << preset.extension << endl;
	}
}
...
{
    cout << "\nUsage: StreamEncoder --frame <width>x<height> --rate <fps> --color <COLOR> --input <yuv-file> --output <filename> [--preset <PRESET>]";
    cout << " [--colors] [--presets]";
    cout << endl;
...
void setDefaultOptions(Options& opt)
{
    ...
    opt.output_file = getExeDir() + "/foreman_qcif.mp4";
...
ErrorCodes prepareOptions(Options &opt, int argc, char* argv[])
{
    ...
    optcfg.addOptions()
            ...
            ("color,c",     opt.yuv_color,  ColorDescriptor(),
                "input color format. Use --colors to list supported color formats.")        
            ("output,o",    opt.output_file,string(), "output file")
            ("preset,p",    opt.preset,     PresetDescriptor(), 
                "output preset. Use --presets to list supported presets.")
