...
void printStatus(const primo::error::ErrorInfo* e)
{
    ...
    else
    {
        ...
        cout << "(facility:" << e->facility() << " error:" << e->code() << " hint: "<< e->hint() << ")" << endl;
    }
...
bool encode_h264_stream(Options& opt)
{
    ...
    if(transcoder->open())
    {
        ...
        const int videoBufferSize = mediaSample->videoBufferSizeInBytes(opt.frame_size.width_, opt.frame_size.height_, opt.yuv_color.Id);
